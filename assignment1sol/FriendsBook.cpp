...
void join(MyADT* theMembers) {
	...
	if ( theMembers->insert(theMember) )
		// Report to user
		cout << "Member: " << theMember.getName() << " has successfully joined FriendsBook." << endl;
	else
	 	cout << "Member: " << theMember.getName() << " has not joined FriendsBook as she/he is already a member!" << endl;
...
void leave(MyADT* theMembers) {
	...
	if ( theMembers->remove(theMember) )
		cout << "Member : " << theMember.getName() << " has now left the network." << endl;
	else
		cout << "Member : " << theMember.getName() << " is not a member!" << endl;	
...
void modify(MyADT* theMembers) {    
    ...
    if ( target != NULL ) {
		...
		// To Do: allows a member to ad more than 1 friend at a time		
...
int main() {
	...
	while (not done) {
		...
		// cout << "\nThere are now " << members->getElementCount() << " members in FriendsBook." << endl;	
		cout << *members << endl;
